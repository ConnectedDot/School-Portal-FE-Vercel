...
export const useGetTeachers = (
  ...
  return {
    ...query,
    data: query.data?.filter((user) => user.role === "TEACHER") as
      | TeacherData[]
      | undefined,
  };
};
...
export const useGetTeacher = (id: string, enabled: boolean = true) => {
  return useGetItem<TeacherData>("/admin/teachers", id, undefined, {
    enabled: !!id && enabled,
    select: (teacher: TeacherData) => ({
      ...teacher,
...
export const useCreateTeacher = (onSuccessFn?: (data: any) => Promise<any>) => {
...
export const useUpdateTeacher = (
  ...
  const client = useQueryClient();
  return useMutation({
    mutationFn: async (body: Partial<TeacherData>) =>
      (await axiosInstance.patch(`/admin/teachers/${id}`, body)).data,
...
export const useGetTeacherProfile = () => {
  return useGetItem<TeacherData>("/teacher/profile", "", undefined, {
    enabled: true,
  });
};
...
export const useUploadTeacherAvatar = () => {
  ...
  return useMutation({
    ...
    onSuccess: () =>
      client.invalidateQueries({ queryKey: ["/teacher/profile"] }),
  });
};
